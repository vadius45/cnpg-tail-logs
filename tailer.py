#!/usr/bin/env python3
"""
pgaudit-forwarder — watches CloudNativePG database pods in a given namespace,
tails their container logs, extracts lines where logger=pgaudit, and writes
them to daily rotating files under /audit/pgaudit/ (served by a PVC).
...
LOG_LABEL_SELECTOR = os.environ.get(
    "POD_LABEL_SELECTOR",
...
def watch_pods(v1: client.CoreV1Api) -> None:
    ...
    try:
        for event in w.stream(
            ...
            namespace=NAMESPACE,
            label_selector=LOG_LABEL_SELECTOR,
            timeout_seconds=0,
        ):
            if _stop.is_set():
                break
...
def main() -> None:
    ...
    logger.info(
        ...
        NAMESPACE, LOG_LABEL_SELECTOR, AUDIT_DIR,
    )
